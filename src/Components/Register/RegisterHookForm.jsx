...
//         name: z.string().nonempty("Please enter your name").min(3, "At least 3 characters").max(30, "Max 30 characters").regex(/^[A-Za-z ]+$/, "Only letters and spaces"),
//         email: z.email({ error: (issue) => issue.input === undefined || issue.input === "" ? "Please enter your email" : "Please enter a valid email format (e.g. user@example.com)" }),
//         password: z.string().nonempty("Please enter your password").regex(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/, "Password must be at least 8 characters, include upper & lower case letters, a number, and a special character"),
//         rePassword: z.string().nonempty("Please enter your confirm password"),
//         dateOfBirth: z.coerce.date({ error: () => "Please enter your Date of Birth" }).refine((value) => {
//             const age = dayjs().diff(dayjs(value), "year"); return age >= 16 && age <= 100;
//         }, { message: "Age must be between 16 and 100 years" }),
//         gender: z.enum(["male", "female"], { error: () => "Please choose your gender" })
